...
import { publicationEnseignantService, socialService, connexionService, profilService } from '../services/api';
...
const EnseignantDashboard = () => {
  ...
  const [stats, setStats] = useState({ publications: 0, connexions: 0, reactions: 0 });
  ...
  const fetchSocialFeed = async () => {
    try {
      ...
      const allPubs = response.data?.publications || [];
      setSocialFeed(allPubs.filter(pub => pub.statut === 'VALIDE'));
    } catch (error) {
  ...
  const fetchSuggestions = async () => {
    try {
      ...
      setRecommendedUsers((response.data || []).filter(u => u.role !== 'ADMIN'));
    } catch (error) {
  ...
  try {
    const profilRes = await profilService.getMonProfil();
    const pubRes = await publicationEnseignantService.getMesPublications(user.id);
    const pubs = pubRes.data.publications || [];

    const connexRes = await connexionService.countConnexions();

    setStats({
      publications: pubs.length,
      connexions: connexRes.data, // 👈 ici
      reactions: pubs.reduce((acc, p) => acc + (p.nombreReactions || 0), 0)
    });
  ...
  const handleDeletePublication = async (id) => {
    ...
    try {
      await publicationEnseignantService.supprimerPublication(user.id, id);
  ...
  const handleSubmitPublication = async (e) => {
    ...
    try {
      ...
      await publicationEnseignantService.creerPublication(user.id, data);
      toast.success('Publication créée ! En attente de validation.');
      setShowPublicationForm(false);
      setPublicationFormData({ titre: '', resume: '', contenu: '', typePublication: 'COURS', fichier: null });
      fetchSocialFeed();
      fetchMyStats();
    } catch (err) {
  ...
  if (!user || loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <p className="text-slate-500 font-bold animate-pulse">Chargement de votre tableau de bord...</p>
    </div>
  );

  const statsDisplay = [
    { label: 'Mes Publications', value: stats.publications, icon: 'article', color: '#0A66C2' },
    { label: 'Mon Réseau', value: stats.connexions, icon: 'group', color: '#057642' },
    { label: 'Total Réactions', value: stats.reactions, icon: 'favorite', color: '#C9222A' },
  ];

  return (
    <div className="w-full pb-12 animate-in fade-in duration-500">
      {/* Hero Header - Style Enseignant */}
      <section className="relative overflow-hidden bg-gradient-to-br from-[#004182] via-[#002244] to-[#001122] p-10 md:p-16 rounded-[2.5rem] mb-10 text-white shadow-2xl shadow-blue-900/20 group">
        <div className="absolute -top-24 -right-24 w-96 h-96 bg-blue-400/10 rounded-full blur-3xl group-hover:scale-110 transition-transform duration-1000"></div>
        <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl group-hover:scale-110 transition-transform duration-1000 delay-150"></div>

        <div className="relative z-10 max-w-3xl">
          <h1 className="text-4xl md:text-6xl font-black mb-6 tracking-tight leading-tight">
            Espace Enseignant, {user?.nomComplet?.split(' ')[0]}
          </h1>
          <p className="text-xl md:text-2xl opacity-90 font-medium leading-relaxed">
            Partagez vos connaissances, collaborez sur des projets de recherche et suivez l'actualité académique.
          </p>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 ring-1 ring-slate-50">
            <div className="flex gap-5 items-center mb-6">
              <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-white shadow-lg ring-4 ring-blue-500/5 bg-slate-100 flex items-center justify-center">
                {user?.photo ? (
                  <img
              ...
              >
                Partagez une ressource ou une annonce...
              </button>
            </div>
            <div className="flex justify-around pt-4 border-t border-slate-50">
              <button onClick={() => setShowPublicationForm(true)} className="flex items-center gap-3 px-5 py-3 hover:bg-blue-50 rounded-2xl text-slate-600 font-black text-xs uppercase tracking-wider transition-all">
                <span className="material-symbols-outlined text-blue-500">article</span> Publication
              </button>
              <button onClick={() => setShowPublicationForm(true)} className="flex items-center gap-3 px-5 py-3 hover:bg-amber-50 rounded-2xl text-slate-600 font-black text-xs uppercase tracking-wider transition-all">
                <span className="material-symbols-outlined text-amber-500">school</span> Cours
              </button>
          ...
          <div className="space-y-8">
            {socialFeed.length === 0 ? (
              <div className="bg-white rounded-[3rem] p-24 text-center border-2 border-dashed border-slate-100 flex flex-col items-center">
                ...
                <h3 className="text-xl font-black text-slate-400">Aucune actualité pour le moment</h3>
                <p className="text-slate-400 mt-2">Le fil d'actualité académique apparaîtra ici.</p>
              </div>
            ) : (
              socialFeed.map((pub) => (
                <div key={pub.id} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 hover:shadow-xl transition-all duration-300 group">
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-full overflow-hidden bg-slate-50 border border-slate-100 shadow-sm flex items-center justify-center">
                        {pub.auteurPhoto ? (
                          <img
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full ring-1 ${
                            pub.auteurRole === 'ETUDIANT' ? 'text-amber-600 bg-amber-50 ring-amber-100' : 'text-blue-600 bg-blue-50 ring-blue-100'
                          }`}>
                            {pub.auteurRole === 'ETUDIANT' ? 'Étudiant' : 'Enseignant'}
                          </span>
                    ...
                    <div className="mb-8 px-2 overflow-hidden rounded-3xl">
                      {(pub.fichierType === 'PNG' || pub.fichierType === 'JPEG' || pub.fichierType === 'JPG' || pub.fichierType === 'GIF') ? (
                        <img 
                          ...
                          alt="Publication" 
                          className="w-full h-auto max-h-[500px] object-cover rounded-3xl border border-slate-100 shadow-sm"
                        />
                      ) : (
                        <a href={`${BASE_URL}${pub.fichierUrl}`} target="_blank" rel="noopener noreferrer"
                          className="flex items-center gap-5 p-6 bg-slate-50 hover:bg-blue-50 rounded-3xl border border-slate-100 transition-all group/file">
                          <div className="w-14 h-14 rounded-2xl bg-white shadow-md flex items-center justify-center group-hover/file:bg-[#0A66C2] group-hover/file:text-white transition-all">
                            <span className="material-symbols-outlined text-2xl font-bold">description</span>
                          </div>
                          <div className="flex-1">
                            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">Document</p>
                            <p className="text-sm font-black text-slate-800 uppercase">Voir la ressource</p>
                          </div>
                  ...
                  <div className="flex items-center justify-between py-6 border-t border-slate-50">
                    <div className="flex items-center gap-8">
                      <button onClick={() => handleReact(pub.id)} className="flex items-center gap-3 group/btn">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center group-hover/btn:bg-blue-50 transition-all">
                          <span className={`material-symbols-outlined text-2xl ${pub.liked ? 'text-blue-600 fill-1' : 'text-slate-400'}`}>thumb_up</span>
                        </div>
                        <span className="text-sm font-black text-slate-700">{pub.nombreReactions}</span>
                      </button>
                      <button className="flex items-center gap-3 group/btn" onClick={() => toggleComments(pub.id)}>
                        <div className="w-12 h-12 rounded-full flex items-center justify-center group-hover/btn:bg-slate-50 transition-all">
                        ...
                        <span className="text-sm font-black text-slate-700">{pub.nombreCommentaires}</span>
                      </button>
                    </div>
                  ...
                  {/* Comments Input */}
                  <div className="flex gap-4 mt-6 px-2 items-center bg-slate-50 p-4 rounded-[2rem] border border-slate-100 focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-500/20 transition-all">
                    <input
                      id={`comment-${pub.id}`}
                      type="text"
                      placeholder="Répondez à cette publication..."
                    ...
                    <button
                      ...
                      className="w-10 h-10 flex items-center justify-center bg-[#0A66C2] hover:bg-blue-700 disabled:bg-slate-200 text-white rounded-full transition-all"
                    >
        ...
        <div className="space-y-10">
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
            <h3 className="text-xs font-black uppercase text-slate-400 tracking-[0.2em] mb-8">Suggestions de réseau</h3>
            <div className="space-y-6">
              {recommendedUsers.map((rec, idx) => (
                <div key={idx} className="flex items-center justify-between group cursor-pointer" onClick={() => navigate('/enseignant/reseau')}>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-black text-slate-900 leading-tight group-hover:text-[#0A66C2] transition-colors">{rec.nomComplet}</p>
                      <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">{rec.role === 'ETUDIANT' ? 'Étudiant' : 'Enseignant'}</p>
                    </div>
                  </div>
            ...
            <form onSubmit={handleSubmitPublication} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <select
                    className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] font-black text-slate-700 outline-none focus:border-[#0A66C2] transition-all"
                  ...
                  >
                    ...
                    <option value="EVENEMENT">Annonce / Événement</option>
                  </select>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <input
                    className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] font-black outline-none focus:border-[#0A66C2] transition-all"
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Contenu</label>
                <textarea
                  className="w-full p-6 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold h-48 outline-none focus:border-[#0A66C2] transition-all resize-none"
              ...
              <div className="flex justify-end gap-6 pt-6">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="bg-[#0A66C2] hover:bg-blue-700 text-white px-12 py-5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-[0.2em] shadow-2xl shadow-blue-500/30 transition-all disabled:bg-slate-200"
                >
